...
public class SecurityConfig {
  ...
  public SecurityFilterChain configureChain(HttpSecurity http) throws Exception {
    ...
    http.csrf(csrf -> csrf
        ...
        .ignoringRequestMatchers("/user/signup", "/login", "/user/email_verify/**", "/chat-test/**", "/chatroom/**", "/chat/list/**")
    );
    ...
    http.authorizeHttpRequests(authorizeRequests -> {
      authorizeRequests
          // 인증 없이 접근 가능한 경로
          .requestMatchers("/user/signup", "/login", "/logout", "/user/email_verify", "/user/email_verify/**", "/events/**", "/participant/**", "/chat/list/**").permitAll()
          // 관리자 권한 필요
