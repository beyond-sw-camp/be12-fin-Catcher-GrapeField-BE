...
public class SecurityConfig {
  ...
  public SecurityFilterChain configureChain(HttpSecurity http) throws Exception {
    ...
    http.authorizeHttpRequests(authorizeRequests -> {
      authorizeRequests
          // 인증 없이 접근 가능한 경로
          .requestMatchers("/user/signup", "/login", "/logout", "/user/email_verify", "/user/email_verify/**", "/events/**", "/participant/**", "/chat/**", "/chat/list/**", "/post/list/**", "/post/**", "/comment/**", "/review/**", "/auth/**", "/search/**","/actuator/**").permitAll()
          .requestMatchers("/swagger-ui.html", "/swagger-ui/**", "/v3/api-docs/**",
              "/v3/api-docs", "/v3/**","/swagger-resources/**", "/webjars/**", "/actuator/health", "/actuator/**", "/actuator/health/**").permitAll()
         // 관리자 권한 필요
